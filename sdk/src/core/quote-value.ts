...
export const MIN_QUOTE_DELIVERY = 0.9;
 ...
 * only as unknown.
 */
export function quoteDeliveryRatio(p: QuoteValueParams): number | null {
  if (!(p.inPriceUsd > 0) || !(p.outPriceUsd > 0)) return null;
  const inUsd = (Number(p.inAmount) / 10 ** p.inDecimals) * p.inPriceUsd;
  const outUsd = (Number(p.outAmount) / 10 ** p.outDecimals) * p.outPriceUsd;
  if (!(inUsd > 0) || !Number.isFinite(outUsd)) return null;
  return outUsd / inUsd;
}
 ...
 */
export function marketLooksBroken(ratio: number | null): boolean {
  return ratio !== null && ratio < MIN_QUOTE_DELIVERY;
}
