import { spendableUsd, type WalletAsset } from "./assets";
...
export const MIN_CONVERT_USD = 0.5;
...
export function convertibleAssets(assets: readonly WalletAsset[], usdcMint: string): WalletAsset[] {
  return assets
    .filter((a) => a.mint !== usdcMint && spendableUsd(a) >= MIN_CONVERT_USD)
    .sort((a, b) => spendableUsd(b) - spendableUsd(a));
}

/** What the wallet holds beyond dollars, valued in dollars. */
export function convertibleUsd(assets: readonly WalletAsset[], usdcMint: string): number {
  return convertibleAssets(assets, usdcMint).reduce((sum, a) => sum + spendableUsd(a), 0);
}
...
export function planTopUp(
  ...
  for (const asset of convertibleAssets(assets, usdcMint)) {
    if (left <= 0) break;
    const available = spendableUsd(asset);
    ...
    const usd = Math.min(available, Math.max(left, MIN_CONVERT_USD));
    legs.push({ asset, usd });
    left -= usd;
  }
  return legs;
