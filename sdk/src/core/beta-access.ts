...
export function parseBetaList(raw: string | undefined): string[] {
  if (!raw) return [];
  return raw
    .split(/[,\s]+/)
    .map((s) => s.trim())
...
export function isBetaIdentity(
  identity: { email?: string | null; wallet?: string | null },
  config: BetaAccessConfig,
): boolean {
  const email = identity.email?.trim().toLowerCase();
  if (email && config.emails.some((e) => e.trim().toLowerCase() === email)) return true;
