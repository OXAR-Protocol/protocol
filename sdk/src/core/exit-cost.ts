 ...
 */
export function exitCostFraction(usdIn: number, usdOut: number | null): number | null {
  if (usdOut === null || !Number.isFinite(usdOut)) return null;
 ...
 */
export function exitCostBand(fraction: number | null): ExitCostBand {
  if (fraction === null) return "none";
  if (fraction <= CHEAP_MAX) return "cheap";
