...
export function tokenDelta(
  pre: readonly TokenBalanceEntry[] | undefined | null,
  post: readonly TokenBalanceEntry[] | undefined | null,
  owner: string,
  mint: string,
): bigint {
  const sum = (entries: readonly TokenBalanceEntry[] | undefined | null): bigint => {
    let total = BigInt(0);
    for (const e of entries ?? []) {
      if (e?.owner !== owner || e?.mint !== mint) continue;
