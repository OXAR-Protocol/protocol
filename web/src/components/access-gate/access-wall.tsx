...
export function AccessWall({ children }: { children: ReactNode }) {
  ...
  if (state === "loading") return <div className="fixed inset-0 bg-paper" />;
  if (state === "unlocked") {
  ...
  return (
    <div className="safe-top fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-paper px-6 text-ink">
      ...
      <div className="relative flex max-w-[560px] flex-col items-center text-center">
        ...
        >
          where does your <span className="italic text-ink/45">money sleep?</span>
        </motion.h1>
        ...
        >
          <input
            ...
            disabled={checking}
            className="h-12 w-full rounded-full border border-ink/15 bg-transparent px-5 lowercase text-[16px] text-ink placeholder:text-ink/30 outline-none transition-colors focus:border-ink/60 disabled:opacity-50"
          />
          <button
            onClick={submit}
