...
interface Props {
  ...
  /** Open the basket. What to DO with it is decided in there. */
  onOpen: () => void;
  onClear: () => void;
}

/**
 * The bar that appears once things are picked.
 *
 * It says what is in the basket and opens it — nothing else. Buying and selling used
 * to be offered here, which put the biggest decision in the app on a strip an inch
 * tall, above a list the thumb was still scrolling. The direction is now chosen once
 * the basket is open, next to the amounts it applies to.
 */
export function PickBar({ picked, selectedCount, heldUsd, state, done, progress, onOpen, onClear }: Props) {
  const { t } = useT();
