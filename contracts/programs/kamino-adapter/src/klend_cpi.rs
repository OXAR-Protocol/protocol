...
// Verified against the on-chain klend IDL (see .kamino-ref/klend-idl.json).
// ============================================================================
...
/// Compute the USDC value of `shares` cTokens from a refreshed klend reserve.
///
...
/// Bundle of accounts the adapter forwards to klend. Holds owned `AccountInfo`
/// clones (cheap — they wrap shared refs) so the caller avoids lifetime tangles
/// with `to_account_info()` temporaries.
pub struct KlendAccounts<'info> {
    pub klend_program: AccountInfo<'info>,
    pub owner: AccountInfo<'info>, // vault PDA — signer propagates from dispatcher
    ...
    pub user_liquidity: AccountInfo<'info>,  // vault_usdc_pool
    pub user_collateral: AccountInfo<'info>, // vault-owned cToken ATA
...
/// a deposit, redeem, or value read. pyth/switchboard oracles are unset for the
/// USDC reserve, so the klend program id is passed as the None placeholder.
///
/// Takes the four accounts it needs directly so `adapter_current_value` (which has
/// no funds-movement accounts) can call it without a full `KlendAccounts`.
...
/// `user_collateral`. `owner` (vault PDA) authorizes the USDC transfer; its signer
/// flag propagates from the dispatcher's `invoke_signed`.
...
/// `user_liquidity`. Symmetric to deposit; `owner` (vault PDA) signs.
